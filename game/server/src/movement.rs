//! Movement systems: turn move requests into paths, advance movers along tiles each tick, snap them
//! to centers, and step them through portals (raising a [`crate::transition::Crossing`] to hand off).

use bevy_ecs::message::Messages;
use bevy_ecs::prelude::*;
use bevy_replicon::prelude::FromClient;
use bevy_time::Time;
use world::actor::{ACTION_RUN, ACTION_WALK, Actor};
use world::area::{self, AreaTag};
use world::combat::is_dead;
use world::core::math::{Direction, Offset, Pos};
use world::core::nav;
use world::core::table::Id;
use world::core::tiling::{Cell, CellPos, TilePos, Tiles, TilesPerSec};
use world::core::time::Seconds;
use world::movement::{MoveRequest, MoveToPortal, Position, position};

use crate::actor::set_facing;
...
fn cross_portal(world: &mut World, entity: Entity) {
    ...
    if dest_area == area_id {
        ...
        world
            .entity_mut(entity)
            .insert(Crossing { dest_area, dest });
        forget(world, entity);
    }
}
